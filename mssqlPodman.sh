...
function startUp()
{
    ...
    echo "#                                              #"
    echo "#          1 ==   Start MSSQL  Podman image    #"
    echo "#                                              #"
    echo "#          2 ==   Stop MSSQL Podman image      #"
    echo "#                                              #"
    echo "#          3 ==   Bash access                  #"            
    echo "#                                              #"
    echo "#          4 ==   SQLCMD                       #"
...
{
    checkPodman
    export mssqlRunning=$(podman ps --no-trunc --format "table {{.ID}}\t {{.Names}}\t" | grep -i MSSQL_DB_Container  | awk '{print $2}' )
...
{   
    ...
    export mssqlRunning=$(podman ps --no-trunc --format "table {{.ID}}\t {{.Names}}\t" | grep -i MSSQL_DB_Container  | awk '{print $2}' )
    export mssqlPresent=$(podman container ls -a --no-trunc --format "table {{.ID}}\t {{.Names}}\t" | grep -i MSSQL_DB_Container  | awk '{print $2}')

    if [ "$mssqlRunning" == "MSSQL_DB_Container" ]; then
        echo "MSSQL podman container is running, please select other option."
        sleep 5
        startUp
    elif [ "$mssqlPresent" == "MSSQL_DB_Container" ]; then
        echo "MSSQL podman container found, restarting..."
        podman restart $mssqlPresent
        countDown
    else
        echo "No MSSQL podman image found, provisioning..."
        podman run -e 'ACCEPT_EULA=Y' -e 'SA_PASSWORD=P@55VV0rd!' -e 'MSSQL_PID=Express' --name MSSQL_DB_Container --network="bridge" -p 1433:1433 -d mcr.microsoft.com/mssql/server:2017-latest-ubuntu 
        export runningAs=$(podman ps --no-trunc --format "table {{.ID}}\t {{.Names}}\t" | grep -i MSSQL_DB_Container  | awk '{print $2}' )
...
{
    checkPodman
    export stopMssql=$(podman ps --no-trunc --format "table {{.ID}}\t {{.Names}}\t" | grep -i MSSQL_DB_Container  | awk '{print $1}' )
    echo $stopMssql
...
{
    checkPodman
    export mssqlImage=$(podman ps --no-trunc --format "table {{.ID}}\t {{.Names}}\t" | grep -i MSSQL_DB_Container  | awk '{print $1}' )
...
{
    checkPodman
    #export mssqlImage=$(podman ps --format "table {{.Image}}\t{{.Ports}}\t{{.Names}}"| grep -i oracle  | awk '{print $4}')
    # this works by greping the known oracle database port
    export mssqlImage=$(podman ps --no-trunc --format "table {{.ID}}\t {{.Names}}\t" | grep -i MSSQL_DB_Container  | awk '{print $1}' )
    podman exec -it -u 0 $mssqlImage /bin/bash
}
...
function showDatabases()
{
    ...
    podman exec $mssqlImage /opt/mssql-tools/bin/sqlcmd -S localhost -U sa -P P@55VV0rd! -Q "SELECT name, database_id, create_date FROM sys.databases;"
}
