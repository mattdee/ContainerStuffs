...
#
   #===================================================================================
   #
   #         FILE: orclEEDocker.sh
   #
   #        USAGE: run it
   #
   #  DESCRIPTION: Run Oracle Enterprise Edition in Docker
   #      OPTIONS:  
   # REQUIREMENTS: Please enable Rosetta for Apple Silcon
   #       AUTHOR: Matt D
   #      CREATED: 01.11.2024
   #      VERSION: 1
   #
...
{
    checkDocker
    export orclRunning=$(docker ps --no-trunc --format "table {{.ID}}\t {{.Names}}\t" | grep -i Oracle_EEDB_Container  | awk '{print $2}' )
...
function installUtils()
{
    ...
    docker exec $orclRunning wget -O /opt/oracle/product/21c/dbhome_1/sqlplus/admin/glogin.sql https://raw.githubusercontent.com/mattdee/orclDocker/main/glogin.sql
    setorclPwd
    startUp
}
...
function startOracle() # start or restart the container named Oracle_EEDB_Container
{   
    checkDocker
    # check to see if Oracle_EEDB_Container is running and if running exit
    export orclRunning=$(docker ps --no-trunc --format "table {{.ID}}\t {{.Names}}\t" | grep -i Oracle_EEDB_Container  | awk '{print $2}' )
    export orclPresent=$(docker container ls -a --no-trunc --format "table {{.ID}}\t {{.Names}}\t" | grep -i Oracle_EEDB_Container  | awk '{print $2}')

    if [ "$orclRunning" == "Oracle_EEDB_Container" ]; then
        echo "Oracle docker container is running, please select other option."
        sleep 5
        startUp
    elif [ "$orclPresent" == "Oracle_EEDB_Container" ]; then
        echo "Oracle docker container found, restarting..."
        docker restart $orclPresent
        countDown
    else
        echo "No Oracle docker image found, provisioning..."
        docker run -d --network="bridge" -p 1521:1521 -p 5500:5500 -p 8080:8080 -it --name Oracle_EEDB_Container container-registry.oracle.com/database/enterprise:latest
...
{
    checkDocker
    export orclImage=$(docker ps --no-trunc --format "table {{.ID}}\t {{.Names}}\t" | grep -i Oracle_EEDB_Container  | awk '{print $2}' )
...
function sysDba()
{
    ...
    (CONNECT_DATA=(SERVER=DEDICATED)(SERVICE_NAME=ORCLCDB)))' as sysdba"
}
